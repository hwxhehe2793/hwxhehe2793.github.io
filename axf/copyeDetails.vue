	...
	export default {
		created() {
			if(this.$route.query.whose === 'bannar') {
				...
				let classifysUrl = 'http://localhost:8081/classifys?product_id=' + this.$route.query.product_id
				this.$http.get(classifysUrl)
					.then((data) => {
						console.log(data.data)
						this.site = data.data
					})
			} else if(this.$route.query.whose === 'shopcart') {
				let shopcartUrl = 'http://localhost:8081/cart'
				this.$http.get(shopcartUrl)
					.then((data) =>{
						this.site = data.data
					})
				
			}else {
				let classifysUrl = 'http://localhost:8081/classifys?id=' + this.$route.query.id
				this.$http.get(classifysUrl)
					.then((data) => {
		...
		computed: {
			siteArr() {
				var _this = this
				if(this.$route.query.whose === 'hotsale') {
					if(this.site.length > 0) {
						for(let i = 0; i < this.site.length; i++) {
							let result = this.site[i].products
							for(let j = 0; j < result.length; j++) {
								if(result[j].product_id == this.$route.query.product_id)
									this.$store.state.detprdid = result[j].product_id
									this.$store.state.detprdarr = result[j]
									return result[j]
							}
						}
					} else { 
				...
				}else if(this.$route.query.whose === 'products'){
					if(this.site.length>0) {
						for(let i = 0; this.site.length; i++){
