		...
		<div class="theme-prd">
			...
			<ul>
				<li v-for="(item,index) in sale">
					 <router-link :to="{path: 'details' , query: { id : classifyid , product_id : item.product_id,whose:'hotsale'}}">
						<div class="theme-prd-left" :style="{backgroundImage:'url(\''+ item.imgs.small +'\')'}"></div>
					</router-link>
...
export default {
	...
	created () {
		...
		this.$http.get(classifysUrl)
			.then((data)=>{
				this.hotsale = data.data[0].products
